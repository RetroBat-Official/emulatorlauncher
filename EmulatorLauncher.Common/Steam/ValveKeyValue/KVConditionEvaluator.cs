...
namespace ValveKeyValue
{
    class KVConditionEvaluator
    {
        ...
        {
            Require.NotNull(definedVariables, "definedVariables");
        ...
        bool EvaluateVariable(string variable) { return definedVariables.Contains(variable); }

        Expression CreateExpression(IList<KVConditionToken> tokens)
        {
            if (tokens.Count == 0)
            {
                throw new InvalidOperationException("CreateExpression called with no condition tokens.");
            }
        ...
        Expression EvaluteVariableExpression(string variable)
        {
            ...
            var method = typeof(KVConditionEvaluator)
                .GetMethod("EvaluateVariable", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        class KVConditionToken
        {
            ...
            public KVConditionTokenType TokenType { get; private set; }

            public object Value { get; private set; }

            public static KVConditionToken Not = new KVConditionToken(KVConditionTokenType.Negation);

            public static KVConditionToken Or = new KVConditionToken(KVConditionTokenType.OrJoin);

            public static KVConditionToken And = new KVConditionToken(KVConditionTokenType.AndJoin);

            public static KVConditionToken LeftParenthesis = new KVConditionToken(KVConditionTokenType.BeginSubExpression);

            public static KVConditionToken RightParenthesis = new KVConditionToken(KVConditionTokenType.EndSubExpression);
        }
    }
}
