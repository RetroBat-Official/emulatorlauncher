...
namespace emulatorLauncher.libRetro
{
    class LibretroControllers
    {
        ...
        private static void WriteHotKeyConfig(ConfigFile config)
        {
            ...
            var hotKey = c0.Input.ToDirectInputCodes(Tools.InputKey.hotkey);
            if (hotKey != null)
            {
                if (hotKey.Type != "key")
//                    config["input_enable_hotkey"] = getConfigValue(hotKey);
  //              else
        ...
        private static string getAnalogMode(Controller controller)
        {
            ...
            {
                var k = controller.Input.ToDirectInputCodes(dirkey);
        ...
        private static Dictionary<string, string> generateControllerConfig(Controller controller)
        {
            ...
            {
                var input = controller.Input.ToDirectInputCodes(btnkey.Key);
                if (input == null)
                    continue;
            ...
            {
                var input = controller.Input.ToDirectInputCodes(btnkey);
                if (input == null)
                    continue;
            ...
            if (controller.Index == 1)
            {
                foreach (var specialkey in retroarchspecials)
                {
                    var input = controller.Input.ToDirectInputCodes(specialkey.Key);
                    if (input == null)
                        continue;
