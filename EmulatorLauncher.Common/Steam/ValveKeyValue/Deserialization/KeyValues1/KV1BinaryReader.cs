...
{
    class BinaryReaderLeaveOpen : BinaryReader
    {
        public BinaryReaderLeaveOpen(Stream input, Encoding encoding) : base(input, encoding)
        {
        }
    ...
    class KV1BinaryReader : IVisitingReader
    {
        ...
        void ReadValue(KV1BinaryNodeType type)
        {
            ...
            switch (type)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException("type", type, "Unhandled binary node type.");
            }
        ...
        KV1BinaryNodeType ReadNextNodeType() { return (KV1BinaryNodeType)reader.ReadByte(); }
    }
}
