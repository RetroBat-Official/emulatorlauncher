...
namespace ValveKeyValue.Deserialization.KeyValues1
{
    sealed class KV1TextReader : IVisitingReader
    {
        ...
        {
            Require.NotDisposed("KV1TextReader", disposed);
            ...
            while (stateMachine.IsInObject)
            {
                ...
                switch (token.TokenType)
                {
                    ...
                    default:
                        throw new ArgumentOutOfRangeException("token.TokenType", token.TokenType, "Unhandled token type.");
                }
            }
        }
        ...
        static KVValue ParseValue(string text)
        {
            ...
            int intValue;
            if (int.TryParse(text, IntegerNumberStyles, CultureInfo.InvariantCulture, out intValue))
            {
            ...
            float floatValue;
            if (!IsStrToLBase10Compatible(text) && float.TryParse(text, FloatingPointNumberStyles, CultureInfo.InvariantCulture, out floatValue))
            {
        ...
        {
            Require.NotNull(hexadecimalRepresentation, "hexadecimalRepresentation");
        ...
        static bool IsStrToLBase10Compatible(string str)
        {
            ...
            while (index < str.Length && str[index] >= '0' && str[index] <= '9')// is '0' or '1' or '2' or '3' or '4' or '5' or '6' or '7' or '8' or '9')
            {
                index++;
            }
