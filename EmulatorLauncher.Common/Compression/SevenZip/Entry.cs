...
namespace EmulatorLauncher.Common.Compression.SevenZip
{
    public class Entry
    {
        ...
        public void Extract(string fileName, ProgressChangedEventHandler progress = null, bool preserveTimestamp = true)
        {
            if (this.IsFolder)
            {
            ...
            {
                this.Extract(fileStream, progress);
            }
        ...
        public void Extract(Stream stream, ProgressChangedEventHandler progress = null, string password = null)
        {
            var callback = new ArchiveStreamCallback(this.index, stream, password);
            if (progress != null)
        ...
        private void Callback_Progress(object sender, ProgressChangedEventArgs e)
        {
